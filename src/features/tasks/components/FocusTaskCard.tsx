...
export const FocusTaskCard: React.FC<TaskProps> = ({ task, onComplete, onEdit, onDelete }) => {
        ...
        <div className="p-4 flex flex-row items-center gap-4">
          ...
          >
            {isCompleted ? (
              <CheckCircle2
                size={24}
                className="text-workspace-green drop-shadow-[0_0_8px_rgba(34,197,94,0.4)]"
              />
            ) : (
              <Circle
                size={24}
                className="text-workspace-text-secondary hover:text-workspace-primary transition-colors"
              />
            )}
          </button>
            ...
            <div className="flex items-center gap-2">
              ...
              {task.priority === 'HIGH' && (
                <Chip
                  size="sm"
                  variant="soft"
                  className="text-[10px] h-5 bg-workspace-red/10 text-workspace-red"
                >
                  High
                </Chip>
              )}
